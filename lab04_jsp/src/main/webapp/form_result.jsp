...
<style>
    <% 
    //(주소줄의) 질의 문자열(query String)에 포함된 요청 파라미터 값을 찾음
...
<%@ include file="header.jspf" %>
    ...
    선언된 지역 변수들.
    스크립트릿에서 지역 변수를 선언 할 때 사용할 수 없는 이름들.(선언되어있기 때문에 이름을 중복해서 선언불가)
    ...
    - session: 세션이 유지되는 동안 정보를 저장하기 위한 객체.
        (예) 로그인 상태 유지.
   ...
   <!-- username의 색 바꾸기 span태그(인라인 태그)로 묶고 id지정해두고 <head>태그 안에서 스타일 줌.-->
   <!-- username이 admin이랑 같으면 <h3>관리자 페이지</h3> 실행.다르면 else문의 HTML코드 실행. -->
   <% if (username.equals("admin")) { //문자열비교는 반드시 equals쓰기. ==등호 사용 X %>
        <h3>관리자 페이지</h3>
   <% } else { %> 
        <h3>일반 사용자 페이지</h3>
   <% } %> 
